...
class MainWindow(QMainWindow):
    ...
    def 좌측상단_좌표_클릭(self):
        def on_click(x, y, button, pressed):
            ...
            print(
                "Button: %s, Position: (%s, %s), Pressed: %s " % (button, x, y, pressed)
            )
            if not pressed:
                return False
    ...
    def btn_click(self):
        ...
        try:
            ...
            while self.num <= self.total_page:
                ...
                with mss.mss() as sct:
                    ...
                    mss.tools.to_png(
                        img.rgb,
                        img.size,
                        output=f"pdf_images/img_{str(self.num).zfill(4)}.png",
                    )

                # 페이지 넘기기
                print(f"{self.num} 페이지 캡쳐 완료!")
                kb_control.press(Key.right)
                kb_control.release(Key.right)
                print(f"{self.num} 페이지 넘김 완료!")
            ...
            cvt_rgb_0.save(
                pdf_name + ".pdf", save_all=True, append_images=img_list, quality=100
            )
            print("PDF 변환 완료!")
